...
1. `npm ci`
2. `npm run build` (`validate-lessons → lint-fix → vite build`。`validate-lessons` が教材 JSON のファイル名・構造を fail-loud 検証し、通過後に `lint-fix` と Vite build が続きます。教材 JSON は `import.meta.glob` により JS バンドルへ inline されます)
