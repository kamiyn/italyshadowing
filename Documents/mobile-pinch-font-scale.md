...
`HomePage.vue` と `ReaderPage.vue` において、スマートホンでも本文フォントサイズを素早く調整できるようにするための実装方針をまとめる。目標 UX は Google Maps のような **2 本指ピンチイン / ピンチアウトでの直接調整** だが、本アプリでは地図の pan / zoom ではなく、既存の `--reader-font-scale` を更新する形で実現する。
...
- スマートホンで 2 本指ピンチによりフォントサイズを直感的に変更できる
