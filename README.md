...
外国語のシャドーイング学習を支援する、シンプルな Vue ベースの静的 Web アプリケーションです。教材 JSON の各行を 1 ページずつ表示し、キーボードだけで前後の行へ移動できます。教材に対応する MP3 音声を端末に読み込めば、再生・速度調整・再生位置に同期した自動ページめくりも行えます。
...
- 教材ごとの MP3 音声を端末に読み込んで再生する (音声はリポジトリに含まれず端末内にのみ保存)
- 再生速度を 0.6〜1.0 倍で調整する (ピッチ維持)
- アプリ内で記録した行頭タイミング (キュー) に沿って、再生に合わせて自動でページを送る
- 現在行の頭から再生し直す / リピートモード (オフ・全体・現在ページ) の切り替え
...
| `←` | 前のページへ移動 |
| `↑` / `Escape` | トップページへ戻る |
| `Enter` | 音声の再生 / 一時停止 (音声読み込み済みのとき) |
| `↓` | リピートモードを循環 (オフ → 全体 → 現在ページ → オフ。キュー未記録の教材では現在ページを飛ばす) |

キュー記録モード中は割り当てが変わります (下記「音声再生と自動ページめくり」参照)。
...
教材ページ下端の操作バーから、教材に対応する MP3 音声を使った学習ができます。

### 音声の読み込み

- 「MP3 を読み込む」からファイルを選択すると、その教材用の音声として端末内 (ブラウザの IndexedDB) に保存されます
- **音声ファイルはリポジトリやサーバーには保存されません**。端末ごとに読み込みが必要です (背景: `Documents/ADR-007-device-local-audio-and-cues.md`)
- 操作バーの `⋯` から差し替え・削除ができます

### 再生操作

- `▶` ボタンまたは `Enter` で再生 / 一時停止。停止中から再生すると現在ページの行頭から始まります
- シークバーで任意の位置へ移動できます
- 速度スライダーで再生速度を 0.6〜1.0 倍に調整できます (ピッチは維持され、設定は端末に保存されます)
- リピートボタン (`🔁` / `🔂`) または `↓` でリピートモードを切り替えます。押すたびに オフ → 全体リピート → 現在ページリピート → オフ と循環します
  - `🔁` 全体リピート: 再生し終わると先頭に戻って再生を続けます (キュー記録がなくても使えます)
  - `🔂` 現在ページリピート: 表示中のページを繰り返します (自動ページ送り用のキューを記録済みのときのみ。未記録の教材では循環時に飛ばされます)
- 再生中にページを移動すると、音声も移動先の行頭へシークします

### キュー記録モード (自動ページめくりの準備)

自動ページめくりには「各行が音声の何秒から始まるか」の記録 (キュー) が必要です。

1. 操作バーの `●` を押すと音声が先頭から再生され、記録モードに入ります
2. 各行の読み上げが始まる瞬間に画面タップまたは `Space` を押します (1 タップ = 1 行)
3. 全行ぶん記録すると自動で保存・終了します。`Escape` または「中止」で破棄して抜けられます (保存済みの既存キューは残ります)

記録中は再生速度を落とすとタップしやすくなります。キューは端末内 (localStorage) に教材ごとに保存され、教材の行数が変わった場合は無効となり再記録が必要です。

記録後は、再生位置が各行の頭に達する約 0.1 秒前に自動でページが送られます。
