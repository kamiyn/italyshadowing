...
- JSON として整形
- アクセントのある母音に対して <b> で囲う
- 同一フォルダにある他の json ファイルを参考にして、1行の長さの調整をする
```
...
- **左余白** (画面幅の 10%): タップで **前のページ** へ戻る
- **それ以外の領域** (テキスト + 右余白): タップで **次のページ** へ進む
- **ページ番号**: タップで **トップページ** へ戻る
...
- トップページのタイトル横にビルド識別子 (コミットハッシュ) が表示されます
- 新しいバージョンがデプロイされても、ブラウザのキャッシュにより古いバージョンが表示され続けることがあります
- 表示が更新されないときは、アプリを閉じて開き直してください
  - Safari タブ表示: タブを閉じて URL を開き直す
  - ホーム画面から起動 (standalone): アプリスイッチャーからアプリを終了して再度開く
